...
func uploadHandler(response http.ResponseWriter, request *http.Request) {
	...
	err = trans.Initialize(newPath, fmt.Sprintf("/m3u8/%s/%s.m3u8", fileName, fileName))
	// trans.MediaFile().SetAspect("640x360")
	trans.MediaFile().SetHlsListSize(0)
	trans.MediaFile().SetHlsSegmentDuration(5)
	trans.FFmpegExec(fmt.Sprintf("-i in/%s.mp4 -profile:v baseline -level 3.0 -s 640x360 -start_number 0 -hls_time 5 -hls_list_size 0 -f hls m3u8s/%s/%s.m3u8", fileName, fileName, fileName))
